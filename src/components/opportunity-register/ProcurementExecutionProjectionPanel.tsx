...
function stageRows(projection: ProcurementExecutionProjection) {
  ...
  return [
    ...
    { title: "Document Preparation", status: projection.documentPreparationStatus, progress: readiness.documentCompleteness, owner: "Staff", due: projection.dueDate, action: "Open Workspace", href: route("/dashboard/deals/[dealId]/execution#document-preparation-workspace"), blockers: projection.blockers.filter((item) => /returnable|document|signature|amendment/i.test(item.problem)) },
