...
export default function VehicleFinanceWorkspace({ initialSection, initialApplicationId }: Props) {
  ...
  async function submitSupplierQuote(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      Object.entries({ ...supplierQuoteForm, quotedAmount: String(Number(supplierQuoteForm.quotedAmount) || 0) }).forEach(([key, value]) => formData.set(key, value));
      if (supplierQuoteFile) formData.set("file", supplierQuoteFile);
      const response = await authFetch(supplierQuoteFile ? "/api/vehicle-finance/supplier-quotes/upload" : "/api/vehicle-finance/supplier-quotes", supplierQuoteFile ? { method: "POST", body: formData } : { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...supplierQuoteForm, quotedAmount: Number(supplierQuoteForm.quotedAmount) || 0 }) });
      const payload = (await response.json().catch(() => null)) as { error?: string } | null;
      ...
      {section === "procurement-cases" ? (
        <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
          <div className="space-y-6">
            ...
            {quoteUploadOpen ? <Card><IdentityCardHeader title="Upload Supplier Quote" subtitle="Internal-only controlled quote upload" /><form className="mt-4 grid gap-3" onSubmit={submitSupplierQuote}>
              ...
              <label className="grid gap-1 text-sm text-slate-300"><span>Original Quote File (PDF, DOCX or XLSX; max 10 MiB)</span><input required type="file" accept=".pdf,.docx,.xlsx,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={(event) => setSupplierQuoteFile(event.target.files?.[0] ?? null)} className="rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white" /></label><button type="submit" disabled={busy === "supplierQuote"} className="rounded-lg bg-cyan-600 px-4 py-2 text-sm font-semibold text-white">{busy === "supplierQuote" ? "Uploading..." : "Upload Supplier Quote"}</button><button type="button" onClick={() => setQuoteUploadOpen(false)} className="text-sm text-slate-300">Cancel</button>
            </form></Card> : null}
          ...
          <Card><IdentityCardHeader title="Procurement Cases" subtitle="Requirement, lifecycle and supplier quote comparison"><Badge tone="neutral">{procurementCases.length} total</Badge></IdentityCardHeader><div className="mt-4 space-y-4">{procurementCases.map((item) => { const client = businessClients.find((businessClient) => businessClient.businessClientId === item.businessClientId); const caseQuotes = supplierQuotes.filter((quote) => quote.procurementCaseId === item.procurementCaseId); return <div key={item.procurementCaseId} className="rounded-lg border border-slate-800 bg-slate-950/70 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-semibold text-slate-100">{item.internalReference} / {item.vehicleQuantity} x {item.make} {item.model}</p><p className="text-xs text-slate-400">{client?.legalName ?? item.businessClientId} / {item.clientRequestor}</p></div><button type="button" onClick={() => { setSupplierQuoteForm((current) => ({ ...current, procurementCaseId: item.procurementCaseId, vehicleDescription: current.vehicleDescription || `${item.make} ${item.model} ${item.variant ?? ""}`.trim() })); setQuoteUploadOpen(true); }} className="rounded-lg border border-cyan-500 px-3 py-2 text-sm font-semibold text-cyan-200">Upload Supplier Quote</button><Badge tone={item.lifecycleStatus === "COMPLETED" ? "success" : item.lifecycleStatus === "CANCELLED" ? "warning" : "info"}>{item.lifecycleStatus}</Badge></div><p className="mt-3 text-sm text-slate-300">{item.requiredSpecifications || item.notes || "No detailed specification recorded"}</p><Table className="mt-4"><thead><tr><th>Supplier</th><th>Quote</th><th>Availability</th><th>State</th><th>Document</th></tr></thead><tbody>{caseQuotes.map((quote) => { const supplier = suppliers.find((entry) => entry.supplierId === quote.supplierId); return <tr key={quote.supplierQuoteId}><td>{supplier?.legalName ?? quote.supplierId}</td><td>{quote.quotedAmount.toLocaleString("en-ZA")}</td><td>{quote.availability}</td><td><Badge tone={quote.quoteState === "SELECTED" ? "success" : quote.quoteState === "DECLINED" ? "warning" : "neutral"}>{quote.quoteState}</Badge></td><td>{quote.supportingDocuments[0] ? <button type="button" onClick={() => window.open(`/api/vehicle-finance/supplier-quotes/${encodeURIComponent(quote.supplierQuoteId)}/documents/${encodeURIComponent(quote.supportingDocuments[0].documentId)}`, "_blank", "noopener,noreferrer")} className="text-cyan-300 underline">View Quote</button> : "No file"}</td></tr>; })}</tbody></Table><p className="mt-3 text-xs text-slate-500">Activity events: {item.activityHistory.length}</p></div>; })}</div></Card>
        </div>
      ) : null}
