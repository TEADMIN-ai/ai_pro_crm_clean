...
import { listDealDocuments } from "@/server/services/dealService";
import ReturnableEvidenceReview from "@/components/opportunity-register/ReturnableEvidenceReview";
...
export default async function DocumentPreparationPage({ params }: { params: Promise<{ dealId: string }> }) {
  ...
  const documents = await listDealDocuments(dealId);
    return <main data-module="dashboard" className="tex-shell grid gap-6">
      ...
      <EnterprisePanel eyebrow="Required evidence" title="Returnable checklist"><div className="grid gap-3 md:grid-cols-2">{view.state.documentChecklist.map((item) => <div key={item.key} className="rounded-md border p-4"><div className="flex items-center justify-between"><h2 className="font-semibold">{item.label}</h2><EnterpriseStatusBadge value={item.status} tone={item.status === "COMPLETE" ? "success" : "warning"} /></div><p className="mt-2 text-sm text-slate-600">{item.required ? "Required" : "Not applicable"}{item.source ? " - " + item.source : ""}</p><Link className="tex-action-button tex-action-button--secondary mt-3 inline-block" href={"/dashboard/deals/" + dealId + "/upload?returnable=" + encodeURIComponent(item.key) + "#" + item.key}>Upload evidence</Link></div>)}</div></EnterprisePanel>
      <ReturnableEvidenceReview dealId={dealId} evidence={documents.filter((document) => Boolean((document as typeof document & { returnableCategory?: string }).returnableCategory)) as Array<{ id: string; name: string; returnableCategory?: string; returnableSubtype?: string | null; status?: string; reviewStatus?: string | null }>} />
