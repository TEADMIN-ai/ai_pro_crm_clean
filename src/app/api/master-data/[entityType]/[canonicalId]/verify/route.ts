...
import { FirestoreMasterDataRepository, actorFromAuthorizedUser, isMasterDataEntityType, updateCanonicalMasterDataEntity, verificationPatch } from "@/lib/master-data";
...
function jsonError(error: unknown) {
  ...
  return NextResponse.json({ error: error instanceof Error ? error.message : "Master Data verification failed." }, { status });
}
...
export async function POST(request: NextRequest, context: { params: Promise<{ entityType: string; canonicalId: string }> }) {
  try {
    ...
    if (previous.status === "archived" || previous.verificationStatus === "REJECTED") {
      return NextResponse.json({ error: "Only active non-rejected Master Data entities can be verified." }, { status: 409 });
    }
    if (!previous.sourceEvidence.length) {
      return NextResponse.json({ error: "Verification requires evidence references." }, { status: 409 });
    }
    const result = await updateCanonicalMasterDataEntity({
      ...
      entityType,
      canonicalId,
      patch: verificationPatch("VERIFIED", user.uid),
      reason: "Master Data API verification.",
    });
