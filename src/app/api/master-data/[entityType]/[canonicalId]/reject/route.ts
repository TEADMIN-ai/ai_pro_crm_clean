...
function jsonError(error: unknown) {
  ...
  return NextResponse.json({ error: error instanceof Error ? error.message : "Master Data rejection failed." }, { status });
}
...
export async function POST(request: NextRequest, context: { params: Promise<{ entityType: string; canonicalId: string }> }) {
  try {
    ...
    const result = await updateCanonicalMasterDataEntity({
      ...
      entityType,
      canonicalId,
      patch: verificationPatch("REJECTED", user.uid),
      reason: "Master Data API rejection.",
    });
