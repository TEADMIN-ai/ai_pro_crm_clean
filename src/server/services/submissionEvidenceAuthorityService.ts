...
export type SubmissionEvidenceType = "PORTAL_RECEIPT" | "SENT_EMAIL" | "SUBMISSION_DOCUMENT";
...
export async function createSubmissionEvidence(input: { dealId: string; actor: AuthorizedUser; type: SubmissionEvidenceType; portalReference?: string | null; note?: string | null; testMarker?: string | null; file?: File | null }) {
  ...
  const db = getFirebaseAdmin(); await db.collection("submissionEvidence").doc(id).set(evidence); await db.collection("commercialAuthorityAuditEvents").add({ action: "submission_evidence_created", entityId: id, actor: input.actor.uid, workspaceId, metadata: { dealId: input.dealId, evidenceType: input.type, testMarker: evidence.testMarker, storagePath }, createdAt: new Date() }); return evidence;
}
