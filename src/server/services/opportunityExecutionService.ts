...
export async function applyOpportunityExecutionAction(input: ActionInput) {
  ...
  if (input.action === "record_submission") { execution.submitted = true; execution.submission = { ...submissionPayload, evidenceReferences: submissionEvidence?.evidenceReferences ?? {} }; execution.submissionCompletionProvenance = { action: "record_submission", status: "SUBMITTED", completedAt: now.toISOString(), completedBy: input.actor.uid, completedByEmail: input.actor.email ?? null, clientQuoteId: asString(submissionPayload.clientQuoteId), tenderPackDocumentId: asString(submissionPayload.tenderPackDocumentId), submissionEvidenceDocumentId: asString(submissionPayload.submissionEvidenceDocumentId) }; patch.status = "submitted"; patch.stage = "submitted"; patch.tenderSubmittedAt = now; patch.tenderSubmittedBy = input.actor.uid; }
  if (input.action === "assign_contractor") {
