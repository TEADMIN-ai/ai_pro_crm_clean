...
import type { RiskRegisterEntry } from "@/types/risk";

describe("buildRiskRegisterSummary", () => {
  test("returns zeroed metrics for an empty register", () => {
    expect(buildRiskRegisterSummary([])).toEqual({
      totalRisks: 0,
      openRisks: 0,
      monitoringRisks: 0,
      mitigatedRisks: 0,
  ...
  test("aggregates dashboard risk metrics from register entries", () => {
    ...
    expect(buildRiskRegisterSummary(risks)).toEqual({
      totalRisks: 3,
      openRisks: 1,
      monitoringRisks: 1,
      mitigatedRisks: 1,
