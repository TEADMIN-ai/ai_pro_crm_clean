...
export function hasValidInternalReviewCompletion(source: Deal | AnyRecord): boolean { const execution = rec(rec(source).opportunityExecution); const provenance = rec(execution.internalReviewApprovalProvenance); return bool(execution.internalReviewApproved) && provenance.action === "complete_internal_review" && provenance.status === "APPROVED" && Boolean(str(provenance.completedAt) && str(provenance.completedBy)); } export function hasValidContractorApproval(source: Deal | AnyRecord): boolean { const execution = rec(rec(source).opportunityExecution); const provenance = rec(execution.contractorApprovalProvenance); return bool(execution.contractorApprovalComplete) && provenance.action === "contractor_approval" && provenance.status === "APPROVED" && Boolean(str(provenance.completedAt) && str(provenance.completedBy)); } export function hasValidSubmissionReviewCompletion(source: Deal | AnyRecord): boolean { const execution = rec(rec(source).opportunityExecution); const provenance = rec(execution.submissionReviewApprovalProvenance); return provenance.action === "complete_submission_review" && provenance.status === "APPROVED" && Boolean(str(provenance.completedAt) && str(provenance.completedBy)); } function hasValidPersistedTimestamp(value: unknown): boolean { const timestamp = str(value); return Boolean(timestamp && Number.isFinite(Date.parse(timestamp))); } export function hasValidSubmissionCompletion(source: Deal | AnyRecord): boolean { const execution = rec(rec(source).opportunityExecution); const provenance = rec(execution.submissionCompletionProvenance); return provenance.action === "record_submission" && provenance.status === "SUBMITTED" && hasValidPersistedTimestamp(provenance.completedAt) && Boolean(str(provenance.completedBy) && str(provenance.clientQuoteId) && str(provenance.tenderPackDocumentId) && str(provenance.submissionEvidenceDocumentId)); } export function isRecoverableLegacyInternalReviewState(source: Deal | AnyRecord): boolean { const root = rec(source); const execution = rec(root.opportunityExecution); const phase = normalizeOpportunityPhase(execution.currentPhase ?? root.workflowStatus); return ["CONTRACTOR_APPROVAL", "PACK_GENERATION", "READY_FOR_SUBMISSION", "SUBMITTED"].includes(phase ?? "") && bool(execution.internalReviewApproved) && !hasValidInternalReviewCompletion(source); } function executionFlags(deal: Deal | AnyRecord) {
  const source = rec(deal);
  const execution = rec(source.opportunityExecution);
  return {
    ...
    contractorApprovalComplete: hasValidContractorApproval(source), submissionReviewApproved: hasValidSubmissionReviewCompletion(source),
    ...
    submitted: hasValidSubmissionCompletion(source),
  };
}
...
export function isDocumentPreparationComplete(state: OpportunityExecutionState): boolean { return state.requirements.reviewed && state.requirements.reviewStatus === "APPROVED" && state.documentChecklist.every((item) => !item.required || item.status === "COMPLETE" || item.status === "NOT_APPLICABLE"); }
...
export function buildOpportunityExecutionState(input: { deal: Deal | AnyRecord; contractor?: AnyRecord | null }): OpportunityExecutionState {
  ...
  addAction(actions, "generate_tender_pack", "Generate Tender Pack", phase === "PACK_GENERATION" && durableClientQuoteReady && uniqueBlockers.length === 0, !durableClientQuoteReady ? "Approved Client Quote must be generated" : uniqueBlockers.join("; ") || "Opportunity is not in pack generation", "/dashboard/tender-pack-requests?dealId=" + encodeURIComponent(String(source.id ?? "")));
