...
export function buildOpportunityExecutionState(input: { deal: Deal | AnyRecord; contractor?: AnyRecord | null }): OpportunityExecutionState {
  ...
  addAction(actions, "prepare_documents", "Prepare Returnables", phase === "DOCUMENT_PREPARATION", phase === "DOCUMENT_PREPARATION" ? null : "Resolve compulsory compliance and pricing blockers first", "/dashboard/deals/" + encodeURIComponent(String(source.id ?? "")) + "/execution#document-preparation-workspace");
