...
const dealId=str(r.dealId)??str(d.id)??str(r.id)??"unknown",opportunityId=str(r.opportunityId)??dealId,contractorId=str(r.contractorId);
const archived=isArchivedSubmissionReviewContractor(c),base=arr(r.blockers),blockers=archived?["Assigned contractor is archived",...base]:base;
const current=stat(r.currentWorkflowPhase??r.currentPhase??r.phase,"COMPLIANCE_REVIEW");
const next=str(r.nextAction)??(blockers.length?"Resolve submission review blockers":"Continue submission review"),assigned=str(r.assignedOwner??r.owner)??owner("overview",r);
...
return{id:str(r.id)??dealId,opportunityId,dealId,opportunityTitle:str(d.title)??str(r.opportunityTitle)??"Untitled opportunity",rfqNumber:str(d.rfqNumber)??str(r.rfqNumber)??str(rec(d.tenderAnalysis).tenderNumber)??dealId,clientIssuer:str(d.clientName)??str(d.issuingAuthority)??str(r.clientIssuer)??str(rec(d.tenderAnalysis).issuingAuthority)??"Unknown issuer",closingDate:str(d.closingDate)??str(d.deadline)??str(r.closingDate),contractorId,contractorName:archived?"Archived contractor":getSubmissionReviewContractorName(c,r),contractorArchived:archived,currentWorkflowPhase:current,readiness:pct(r.readiness??r.readinessScore),complianceStatus:stat(r.complianceStatus),pricingStatus:stat(r.pricingStatus),boqStatus:stat(r.boqStatus),documentStatus:stat(r.documentStatus),signatureStatus:stat(r.signatureStatus),approvalStatus:stat(r.approvalStatus),packStatus:stat(r.packStatus),submissionStatus:stat(r.submissionStatus,"not_submitted"),blockers,nextAction:next,assignedOwner:assigned,updatedAt:str(r.updatedAt),workspaceId:str(r.workspaceId)??str(d.workspaceId),connectionStatus:contractorId&&dealId!=="unknown"?"connected":"incomplete",sections,auditTimeline:(input.activity??[]).map((x,i)=>{const a=rec(x);return{id:str(a.id)??`activity-${i+1}`,message:str(a.message)??str(a.type)??"Submission review updated",phase:str(a.phase),actor:str(a.performedByEmail??a.actorEmail??a.actor),createdAt:str(a.createdAt)}})}}
