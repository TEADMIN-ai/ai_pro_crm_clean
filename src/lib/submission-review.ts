...
blockers:string[];nextAction:string;assignedOwner:string;updatedAt:string|null;workspaceId:string|null;taxDocumentStatus:string;sarsVerificationStatus:string;sarsVerifiedAt:string|null;sarsRecheckDueAt:string|null;sarsIdentityMatch:string;sarsVerificationBlockers:string[];sarsVerificationRoute:string|null;sarsVerifiedByName:string|null;sarsVerificationSource:string|null;sarsEvidenceAvailable:boolean;
...
const current=executionState.currentPhase;
const next=projection.nextAction.label??(blockers.length?"Resolve submission review blockers":"Continue submission review"),assigned=str(r.assignedOwner??r.owner)??projection.assignedOwner??owner("overview",r);
...
section("reviewApproval","Review & Approval",projection.reviewStatus === "complete" ? "COMPLETE" : "pending",/approval|review|signature/i,evidence(r.approvalEvidence),stat(r.approvalNextAction,"Complete internal approval")),
...
return{id:str(r.id)??dealId,opportunityId,dealId,opportunityTitle:str(d.title)??str(r.opportunityTitle)??"Untitled opportunity",rfqNumber:str(d.rfqNumber)??str(r.rfqNumber)??str(rec(d.tenderAnalysis).tenderNumber)??dealId,clientIssuer:str(d.clientName)??str(d.issuingAuthority)??str(r.clientIssuer)??str(rec(d.tenderAnalysis).issuingAuthority)??"Unknown issuer",closingDate:str(d.closingDate)??str(d.deadline)??str(r.closingDate),contractorId,contractorName:archived?"Archived contractor":getSubmissionReviewContractorName(c,r),contractorArchived:archived,currentWorkflowPhase:current,readiness:projection.submissionReadiness ?? executionState.readiness,complianceStatus:stat(projection.complianceStatus),pricingStatus:stat(projection.pricingStatus),boqStatus:stat(projection.pricingClassification),documentStatus:stat(projection.documentPreparationStatus),signatureStatus:stat(projection.signatureStatus),approvalStatus:projection.reviewStatus === "complete" ? "COMPLETE" : "pending",packStatus:stat(projection.packStatus),submissionStatus:stat(r.submissionStatus??projection.submissionStatus,"not_submitted"),blockers,nextAction:next,assignedOwner:assigned,updatedAt:str(r.updatedAt),workspaceId:str(r.workspaceId)??str(d.workspaceId),taxDocumentStatus:projection.taxDocumentStatus,sarsVerificationStatus:projection.sarsVerificationStatus,sarsVerifiedAt:projection.sarsVerifiedAt,sarsRecheckDueAt:projection.sarsRecheckDueAt,sarsIdentityMatch:projection.sarsIdentityMatch,sarsVerificationBlockers:projection.sarsVerificationBlockers,sarsVerificationRoute:projection.sarsVerificationRoute,sarsVerifiedByName:projection.sarsVerifiedByName,sarsVerificationSource:projection.sarsVerificationSource,sarsEvidenceAvailable:projection.sarsEvidenceAvailable,connectionStatus:contractorId&&dealId!=="unknown"?"connected":"incomplete",sections,auditTimeline:(input.activity??[]).map((x,i)=>{const a=rec(x);return{id:str(a.id)??`activity-${i+1}`,message:str(a.message)??str(a.type)??"Submission review updated",phase:str(a.phase),actor:str(a.performedByEmail??a.actorEmail??a.actor),createdAt:str(a.createdAt)}})}}
