# TEOS Documentation Index

This repository now treats the following folders as the governing TEOS documentation set:

- `docs/architecture`
- `docs/engineering`
- `docs/roadmap`
- `docs/decisions`

## Canonical Governance
- [TEOS Master Engineering Charter](./architecture/TEOS_MASTER_ENGINEERING_CHARTER.md)
- [System Architecture](./architecture/SYSTEM_ARCHITECTURE.md)
- [Authentication Architecture](./architecture/AUTHENTICATION_ARCHITECTURE.md)
- [Multi-Workspace Architecture](./architecture/MULTI_WORKSPACE_ARCHITECTURE.md)
- [Database Architecture](./architecture/DATABASE_ARCHITECTURE.md)
- [API Architecture](./architecture/API_ARCHITECTURE.md)
- [AI Architecture](./architecture/AI_ARCHITECTURE.md)
- [Coding Standards](./engineering/CODING_STANDARDS.md)
- [UI Design System](./engineering/UI_DESIGN_SYSTEM.md)
- [Testing Strategy](./engineering/TESTING_STRATEGY.md)
- [Security Guidelines](./engineering/SECURITY_GUIDELINES.md)
- [Deployment Guide](./engineering/DEPLOYMENT_GUIDE.md)
- [Product Roadmap](./roadmap/PRODUCT_ROADMAP.md)
- [Release Plan](./roadmap/RELEASE_PLAN.md)
- [Sprint Board](./roadmap/SPRINT_BOARD.md)
- [Changelog](./roadmap/CHANGELOG.md)
- [ADR-001: TEOS Platform Direction](./decisions/ADR-001-TEOS-PLATFORM.md)

## Legacy References
The following documents remain as historical or contextual references:

- `docs/architecture.md`
- `docs/data-model.md`
- `docs/PRODUCTION.md`
- `docs/implementation-timeline.md`
- `docs/developer-notes/*`
- `docs/runbooks/*`
